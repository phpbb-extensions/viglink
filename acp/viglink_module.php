...
class viglink_module
{
	...
	public function main($id, $mode)
	{
		...
		$sub_id = md5($config['viglink_api_servername'] . $config['questionnaire_unique_id']);
		...
		{
			$convert_account_link = @file_get_contents('https://www.phpbb.com/viglink/convert?domain=' . urlencode($config['server_name']) . '&amp;siteid=' . $config['viglink_api_servername'] . '&amp;uuid=' . $config['questionnaire_unique_id'] . '&amp;key=' . $config['phpbb_viglink_api_key']);
